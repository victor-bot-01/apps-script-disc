...
const CONFIG = {
  // Nome da Script Property que guarda a chave da API.
  API_KEY_PROP: "SHEETS_API_KEY",
...
function ping_(method, authenticated) {
  const keyLen = String(PropertiesService.getScriptProperties().getProperty(CONFIG.API_KEY_PROP) || "").length;

  const base = {
    version: SCRIPT_VERSION,
    method,
    authenticated: !!authenticated,
    apiKeyConfigured: keyLen > 0,
...
function checkApiKey_(key) {
  const expected = String(
    PropertiesService.getScriptProperties().getProperty(CONFIG.API_KEY_PROP) || ""
  ).trim();
  ...
  if (!expected) {
    return {
      ...
      hint: `Configurações do projeto → Propriedades do script → adicionar "${CONFIG.API_KEY_PROP}".`,
      version: SCRIPT_VERSION,
    };
  }

  if (!key || key !== expected) return { ok: false, error: "Invalid key" };
...
function verificarConfiguracao() {
  ...
  const key = String(PropertiesService.getScriptProperties().getProperty(CONFIG.API_KEY_PROP) || "");
  linhas.push(`Versão do script: ${SCRIPT_VERSION}`);
  linhas.push(key
    ? `✅ ${CONFIG.API_KEY_PROP} configurada (${key.length} caracteres)`
    : `❌ ${CONFIG.API_KEY_PROP} NÃO configurada — Configurações do projeto → Propriedades do script`);
